...
static void lcd_main_menu()
{
	...
	MENU_ITEM(submenu, MSG_LIGHT, lcd_control_light);
#ifdef SDSUPPORT
    if (card.cardOK)
    {
        if (card.isFileOpen())
...
static void lcd_control_light(){
	MENU_ITEM(gcode, MSG_LIGHT_OFF, PSTR("M84"));
	MENU_ITEM(gcode, MSG_LIGHT_LOW, PSTR("M84"));
	MENU_ITEM(gcode, MSG_LIGHT_MED, PSTR("M84"));
	MENU_ITEM(gcode, MSG_LIGHT_FULL, PSTR("M84"));
